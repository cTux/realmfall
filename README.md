...
The pre-commit hook runs `pnpm check:version`, `pnpm typecheck`, and `pnpm quality:staged`, then refreshes the Git index for auto-fixed tracked files. `pnpm quality:staged` runs `oxlint --fix -c .oxlintrc.json` only on staged JavaScript and TypeScript files first, then uses `concurrently` to run Stylelint on staged `src` CSS and SCSS files in parallel with either `vitest related` for staged source, runtime JSON content, and test files or the full `pnpm test` suite when shared test inputs such as `package.json`, `pnpm-lock.yaml`, `vite.config.ts`, TypeScript config, or `src/test/setup.ts` are staged. `pnpm check:version` still requires `package.json` to advance by patch version relative to `HEAD`.
