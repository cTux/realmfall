...
- When the active mood changes between ambient, combat, dungeon, or town, the current track crossfades immediately into a newly selected track from the next playlist instead of waiting for the old track to finish.
- When a track ends within the same mood, the controller starts the next selected track from that mood with a short fade-in and preserves the no-repeat-until-cycle-complete playlist behavior.
