...
- The hex info window can be opened manually from the dock or the `C` hotkey even on empty hexes; structure, loot, and combat auto-open or auto-close rules only take over when the player has not manually pinned that window open.
