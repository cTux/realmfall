...
- The crafting tab is sourced from the live generated crafting registry plus the cooking and smelting registries; it does not carry a separate themed expansion recipe table, and its total recipe count matches the total equippable icon-variation count exactly.
...
- Generated crafting icon outputs reuse the canonical generated-equipment family manifest for slot, category, icon-pool, offhand-occupancy, and granted-ability metadata, while the crafting registry keeps only the workshop-specific stat floor and key-prefix details.
