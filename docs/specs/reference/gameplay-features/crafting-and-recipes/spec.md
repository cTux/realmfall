...
- Workshop material costs are scaled 10x from their base requirements. Smelting recipes now refine ore into matching ingots at a 1:1 ratio after scaling, so one ingot craft consumes ten ore while keeping the existing lower fuel quantities as alternative single-source requirements. Cooking recipes keep one-to-one food ingredient quantities so one raw source ingredient still produces one cooked result.
