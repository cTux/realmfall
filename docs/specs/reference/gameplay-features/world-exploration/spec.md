...
- Each approved movement step updates the player position immediately in state, then runs a `1000 ms` world-slide animation that moves the map behind the player while outgoing edge hexes fade away, newly visible edge hexes slide into view from the new origin, and both transition edge sets keep their correct fog-of-war treatment.
