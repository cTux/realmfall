...
This spec covers the desktop-style game settings window, its dock and hotkey access, the persisted graphics, audio, interface, and gameplay controls exposed there, and the current save-area reset flow.
...
- The current settings content uses a reusable horizontal tab row and exposes `Graphics`, `Audio`, `Interface`, `Gameplay`, and `Saves` tabs.
- The settings window keeps the tab-specific control markup in neighboring graphics, audio, interface, gameplay, and saves panel components so the lazy-loaded content shell only owns draft state, tab switching, and save or reset actions.
...
- The graphics tab annotates renderer presets, the Pixi render-FPS control, and each renderer toggle with a `Performance impact` label that uses `Low`, `Medium`, and `High` severity copy with matching green, orange, and red emphasis on the severity value.
...
- The `Saves` tab surfaces separate reset actions for `Adventure`, `Window layout`, `Audio settings`, `Graphics settings`, `Interface settings`, `Gameplay settings`, and `World map`.
