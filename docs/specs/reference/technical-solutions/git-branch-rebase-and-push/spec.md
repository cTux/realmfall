...
This spec covers the repository-local helper that rebases the current branch onto `origin/master`, auto-resolves package version conflicts, and republishes the rewritten branch.
...
- It rebases with `git pull origin master --rebase`, matching the contributor workflow the repository uses for refreshing feature branches from the main line.
