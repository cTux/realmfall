...
- Terrain, structures, loot, factions, dungeon worlds, and many event outcomes rely on deterministic noise or seeded RNG helpers.
...
- World-generated loot families resolve from weighted `core.config.ts` chance maps, keeping consumables common while preserving lower-frequency weapon, armor, offhand, and artifact rolls on deterministic world caches.
...
- Dungeon and blood-moon systems layer additional scaling on top of the base tier model, but the blood moon, harvest moon, and earthshake event loops mutate only the surface world.
