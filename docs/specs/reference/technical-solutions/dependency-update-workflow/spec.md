...
- By default, the mutating update scripts stage `package.json` and `pnpm-lock.yaml`, then delegate to `pnpm git:commit` with a conventional `chore(dependencies)` message so dependency refresh commits use the same repository commit wrapper as ordinary local commits without adding a separate version-only diff.
