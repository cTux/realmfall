...
- By default, the mutating update scripts stage `package.json` and `pnpm-lock.yaml`, then delegate to `pnpm git:commit` with a conventional `chore(dependencies)` message so dependency refresh commits keep the patch-version bump rule aligned with ordinary local commits.
