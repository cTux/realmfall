...
- The log window caches parsed timestamp, message, and display-text metadata by log entry object and keeps the typing animation state inside the animated newest row, so appending a log does not force older rows to reparse or the full list to tick on every typing step.
