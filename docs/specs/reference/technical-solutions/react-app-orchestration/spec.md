...
- `useGameActionHandlers` delegates repeated logged-command description shapes to neighboring `src/app/App/hooks/gameActionHandlers/*` helpers, so the hook stays focused on pause gating and handler wiring instead of rebuilding the same command-log adapters inline.
