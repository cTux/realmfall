...
- The bootstrap shell reads interface-setting defaults and world viewport
  minimums from tiny entry-safe modules instead of the broader
  window-registry config, keeping startup settings normalization and initial
  Pixi sizing detached from icon-heavy app wiring.
...
- `useAppGameView` now receives explicit gameplay slices such as `player`, `tiles`, `logs`, `combat`, `enemies`, `seed`, and `worldDayIndex` instead of a broad `game` object, and nearby selectors such as `getTownStockForDay` expose matching narrow state inputs when the UI only needs a subset of `GameState`.
- `useAppWindowViews` receives the specific gameplay slices it reads, such as `playerState`, `combatState`, `homeHex`, and `tileLootSnapshot`, so the window composition layer does not depend on the full `game` object or mislabeled inventory-shaped loot state.
...
- `AppWindows` memoizes the deferred-window view and action slices, while fixed and deferred window composition share `useDetailTooltipHandlers` so memoized window shells do not receive fresh detail-tooltip wrapper objects from unchanged parent renders.
...
- `useCombatAttentionWindow` owns the player-initiated auto-open hex-content behavior that reacts to combat entry on movement, while enemy-initiated chase combat leaves the hex-content window closed so `App.tsx` does not keep lifecycle refs and transition bookkeeping for that single window rule.
- `useCombatHexInfoPersistence` keeps the hex-content window open across the first post-combat render whenever that window was already visible during the fight, preventing the empty-hex auto-close path from collapsing combat context as the encounter clears.
- `useHexInfoWindowPromotion` owns the structure, loot, and combat auto-open or auto-close rules for hex content while preserving manual open or close overrides for the current hex-context until that context changes.
...
- World movement, pending-combat auto-start, and combat automation now share a worker-backed gameplay transition source with a local fallback, keeping the deterministic simulation entrypoints off the main thread while preserving the existing app-level orchestration contracts.
