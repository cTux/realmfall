...
- `pnpm git:commit` auto-bumps the patch version in `package.json` when the working tree has not already moved past `HEAD`, stages `package.json` even when the version already moved past `HEAD`, and then delegates to `git commit` so ordinary commits and retry commits keep the version progression in the index.
