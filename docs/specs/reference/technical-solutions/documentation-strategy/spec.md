...
- AI instruction entrypoints such as `AGENTS.md`, `CLAUDE.md`, and `.github/copilot-instructions.md` are synchronized through `pnpm sync:ai-entrypoints` and stay as thin loaders that point back to the canonical rules instead of telling every task to preload supporting docs.
- Serena project configuration is treated as repository-owned only at `.serena/project.yml`; local Serena overrides, memories, caches, and other runtime artifacts remain ignored unless a task explicitly introduces a shared Serena memory file.
