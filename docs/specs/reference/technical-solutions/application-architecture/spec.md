...
- Player-derived reads use narrower helpers in `src/game/progression.ts`: `getPlayerCombatStats` serves combat and survivability consumers, `getPlayerProgressionSummary` serves level or XP displays, and `getPlayerOverview` remains the broad hero-facing snapshot for app windows that need both.
