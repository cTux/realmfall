...
- App, UI, i18n, normalization, and related test files now import gameplay-facing types and canonical registries through `src/game/stateTypes.ts` or the owning focused domain module instead of importing `src/game/types.ts` directly.
...
- `src/game/stateRewards.ts` stays the stable reward entrypoint, while `src/game/stateRewards/gathering.ts`, `src/game/stateRewards/enemyLoot.ts`, and focused sibling files (`enemyLootDropSink.ts`, `enemyLootItemDrops.ts`, `enemyLootGoldAndUtilityDrops.ts`, `enemyLootBloodMoonDrops.ts`, `enemyLootSkinning.ts`) own the two main reward families behind that facade.
