...
- The staged-quality path formats staged Prettier-supported files first, excluding SVG assets because the current formatter toolchain does not parse them, scopes Oxlint auto-fixes to staged JavaScript and TypeScript files, scopes Stylelint to staged `src` CSS and SCSS files, and scopes Vitest to tests related to staged source files, runtime JSON content, or test files.
- The pre-push path owns the full-project `pnpm typecheck`, `pnpm lint`, `pnpm test`, and `pnpm build` gates, while version-only `package.json` diffs remain on the staged path and shared test-input changes route full-suite validation to pre-push instead of every commit.
