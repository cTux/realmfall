...
- The pull-request workflow reports startup delivery budgets through `pnpm build:budget`, which runs a production build, reads the Vite manifest, and fails if the bootstrap graph or its key chunks exceed the current thresholds.
