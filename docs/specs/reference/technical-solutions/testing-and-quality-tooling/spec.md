...
- The pull-request workflow restores and saves `.tests/vitest-cache` with `actions/cache` in the `test-node` job, hashes the Vite and TypeScript config inputs plus shared Vitest setup files, declares explicit read-only `contents: read` permissions, keeps checkout credentials disabled, skips documentation-only diffs, and runs `typecheck-and-lint`, `test-node`, and strict bundle-budget `build` as independent jobs.
