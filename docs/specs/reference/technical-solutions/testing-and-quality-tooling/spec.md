...
- The repository still keeps ESLint and Biome available for comparison work, but they are secondary tools rather than the default quality gate.
...
- The pre-commit workflow keeps full-project typecheck global, scopes Oxlint auto-fixes to staged JavaScript and TypeScript files as the first staged step, and then uses `concurrently` to run staged Stylelint and staged or full Vitest work in parallel when both queues are present.
