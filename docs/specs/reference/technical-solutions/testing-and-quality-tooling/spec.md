...
- A Storybook parity test guards that each top-level component directory in `src/ui/components` and each top-level standalone PascalCase component file keep a story, while helper JSX modules outside that component naming convention are not treated as standalone Storybook surfaces. The same test also keeps the entity catalog stories connected to the live config-derived fixtures.
