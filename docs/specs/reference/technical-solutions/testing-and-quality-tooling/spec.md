...
- The startup budget thresholds now account for the production-only client obfuscation pass while still keeping explicit ceilings on `index`, `App`, `react-core`, `react-dom-vendor`, `state`, `en`, `pixi`, and total startup JavaScript.
