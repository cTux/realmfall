...
- Shared-package and server root-module coverage now moves into sibling `__tests__/` directories with focused `*.spec.test.ts(x)` files plus feature-owned `*Testkit.ts(x)` entrypoints, keeping even non-browser suites aligned with the same intent-first testkit convention instead of leaving root-level `*.test.ts` files in place.
