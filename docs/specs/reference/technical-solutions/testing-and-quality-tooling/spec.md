...
- `scripts/tests/app-ui-state-boundaries.test.ts` guards that non-test modules under `src/app` and `src/ui` do not drift back to broad `src/game/state.ts` imports when narrower gameplay modules already cover the same read-only types or selectors.
