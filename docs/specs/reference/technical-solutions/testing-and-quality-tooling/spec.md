...
- The pre-commit workflow keeps full-project typecheck global but scopes ESLint auto-fixes to staged JavaScript and TypeScript files, scopes Stylelint to staged `src` CSS and SCSS files, and scopes Vitest to tests related to staged source or test files.
