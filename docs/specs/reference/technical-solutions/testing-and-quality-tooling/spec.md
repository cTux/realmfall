...
- A Storybook parity test guards that each top-level client component directory in `packages/client-web/src/ui/components` and each top-level standalone PascalCase component file keep a story, while shared component directories migrated into `packages/ui-react/src/components` can satisfy that parity through the shared package story tree. Helper JSX modules outside that component naming convention are not treated as standalone Storybook surfaces, and the same test also keeps the entity catalog stories connected to the live config-derived fixtures.
