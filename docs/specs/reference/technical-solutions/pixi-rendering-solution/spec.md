...
- Cloud animation layers follow world-map pan and movement transitions at half-speed parallax, wrap their offset through the cloud travel span so long-distance travel does not push the entire layer off-screen, render pronounced lower shadow drops than the visible cloud sprites, and honor live surface-cloud visibility plus transparency settings without recreating the Pixi app.
...
- Terrain background PNGs are packed through `pnpm build:client:assets`, which emits `src/assets/generated/world-terrain-atlas.png` and `src/assets/generated/world-terrain-atlas.json` from the canonical terrain source list.
