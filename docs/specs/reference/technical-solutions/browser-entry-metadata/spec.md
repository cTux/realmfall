...
- `index.html` modulepreloads the deferred `src/app/App/index.ts` entry so the
  browser can fetch the App chunk during locale loading, while `src/main.tsx`
  starts the runtime `import('./app/App')` work before locale and font
  readiness settle and keeps only the final live render gated behind those
  prerequisites. The bootstrap path reads interface-setting defaults and world
  viewport minimums from tiny entry-safe modules, so startup does not import
  the broader icon-bearing window-registry config just to normalize settings or
  size the first Pixi canvas.
