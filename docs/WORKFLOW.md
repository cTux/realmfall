...
- Check the matching package README when the task is scoped to `packages/client-web`, `packages/common`, `packages/core`, `packages/server-auth`, `packages/server-chat`, `packages/server-world`, or `packages/ui-react`.
...
- `pnpm dev` and `pnpm serve` orchestrate the client and `server-world` only. Start `server-auth` and `server-chat` through their filtered package commands when a task needs those services.
- Run targeted tests and any area-specific commands during development when they help you iterate. Prefer `pnpm --filter @realmfall/client-web test:node` for client gameplay, persistence, i18n, and script coverage, and `pnpm --filter @realmfall/client-web test:jsdom` for shared UI plus client React, Pixi, and other browser-surface coverage.
- Use `pnpm typecheck` for the shared workspace typecheck path across `packages/common`, `packages/core`, `packages/server-world`, `packages/server-auth`, `packages/server-chat`, `packages/ui-react`, and `packages/client-web`.
- Use `pnpm lint` for the shared workspace lint path across `packages/common`, `packages/core`, `packages/server-world`, `packages/server-auth`, `packages/server-chat`, `packages/ui-react`, and `packages/client-web`.
- Use `pnpm build` for the shared workspace build path across `packages/common`, `packages/core`, `packages/server-world`, `packages/server-auth`, `packages/server-chat`, `packages/ui-react`, and `packages/client-web`.
- Use `pnpm test` for the shared automated test path across `packages/core`, `packages/server-world`, `packages/server-auth`, `packages/server-chat`, `packages/ui-react`, and `packages/client-web`, with the client package running both `test:jsdom` and `test:node`.
