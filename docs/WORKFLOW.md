...
- Keep `pnpm lint` mapped to the enforced ESLint workflow unless the repository explicitly migrates away from it. Additional linters such as Oxlint and Biome may be added alongside ESLint for comparison, but they should not silently replace the current quality gate.
