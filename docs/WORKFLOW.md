...
- Keep React hook checks enforced through Oxlint on the TypeScript and TSX path so pre-commit still catches invalid hook usage and missing effect dependencies.
- When replacing configured gameplay content, update canonical ids and live references directly instead of adding display-name fallback paths to preserve removed content.
- When adding or changing gameplay iconography, vendor the SVGs in the repository and update runtime icon references to local assets instead of external URLs.
