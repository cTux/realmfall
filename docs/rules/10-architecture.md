...
- Keep canonical runtime ability definitions and lookup in `src/game/abilityCatalog.ts`. Put enemy loadout selection, equipped-ability sorting, and similar assignment helpers in `src/game/abilityRuntime.ts` or narrower neighboring modules instead of combining both responsibilities in one file.
