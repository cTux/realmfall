...
- Keep repo-native Codex skills in committed `.codex/skills/` and keep `docs/realmfall-skills.md` as the routing index for them. Do not commit installer-managed `.agents/skills/` output from `skills.sh`; audit external skills, then adapt the durable guidance into repo-owned skill files or references.
...
- For worktree-heavy local Serena usage, prefer a shared external project data folder configured through global `project_serena_folder_location` so onboarding and memories are reused across worktrees without committing runtime artifacts.
