...
- Keep view-model hooks such as `useAppGameView` on explicit gameplay slices like `player`, `tiles`, `logs`, `combat`, and `worldTimeMs`. If one selector is the blocker, narrow that selector's state shape instead of widening the whole hook back to `GameState`.
