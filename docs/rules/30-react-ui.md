...
- When `useGameActionHandlers` repeats localized `createLoggedGameTransition` wrappers, move the command-description builders into neighboring helpers under `src/app/App/hooks/gameActionHandlers/` and keep the hook focused on pause gating plus its public handler contract.
