...
- When `useGameActionHandlers` repeats localized `createLoggedGameTransition` wrappers, model the repeated inventory, town-stock, static, equipment-slot, and debug command families as local descriptor records plus focused builders under `src/app/App/hooks/gameActionHandlers/`, and keep the hook focused on pause gating, timed transition application, and its public handler contract.
...
- Keep new localized copy clear and fully written. If a clear localization addition pushes a locale payload over a tracked budget, raise that budget instead of shortening the copy into unclear wording.
