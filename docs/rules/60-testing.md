...
- Keep `pnpm build` on the shared workspace build path across `packages/common`, `packages/server`, `packages/ui`, and `packages/client`, and keep `pnpm build:budget:strict` aligned with that same workspace build path before it applies the client startup-budget gate.
