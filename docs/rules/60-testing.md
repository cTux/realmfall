...
- When a root module under `packages/server/src` or `packages/ui/src` adopts the testkit pattern, move the coverage into a sibling `__tests__/` directory with focused `*.spec.test.ts(x)` files plus a feature-owned `*Testkit.ts(x)` entrypoint instead of leaving bare root-level `*.test.ts` files beside the source module.
...
- Keep `pnpm build` on the shared workspace build path across `packages/common`, `packages/server`, `packages/ui`, and `packages/client`.
