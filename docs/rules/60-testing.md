...
- Keep automatic `package.json` patch-version bumps on the scoped pre-commit path. A staged `package.json` diff that only updates the `version` field should stay lightweight release metadata rather than a reason to rerun the full Vitest suite at commit time.
