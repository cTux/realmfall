...
- Keep `pnpm build` on the shared workspace build path across `packages/common`, `packages/server`, `packages/ui`, and `packages/client`, while strict startup-budget enforcement remains on `pnpm build:budget:strict`.
