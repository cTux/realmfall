...
- React app orchestration is more focused than it was. `src/app/App/useAppControllers.ts`, `src/app/App/useAppGameView.ts`, `src/app/App/useWindowTransitions.ts`, and `src/app/App/useAppWindowViews.ts` now hold most controller, selector, transition, and window-view logic instead of expanding `App.tsx` and `AppWindows.tsx` further.
