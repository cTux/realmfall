...
describe('renderScene cache invalidation', () => {
  ...
  it('rerenders cached marker text when the active interface font changes inside the same frame bucket', async () => {
    const { renderScene } = await import('./renderScene');
    const { getSceneCache } = await import('./renderSceneCache');
    const { applyInterfaceFontFamily } =
      await import('../../app/interfaceFonts');
    const game = createGame(2, 'render-scene-font-cache');
    game.tiles['1,0'] = {
      coord: { q: 1, r: 0 },
      terrain: 'plains',
      items: [],
