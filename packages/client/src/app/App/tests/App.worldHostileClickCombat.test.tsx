...
describe('App world hostile click combat', () => {
  ...
  it('walks only to the safe staging tile for a distant hostile click, then auto-steps after victory and enforces movement cooldown', async () => {
    ...
    try {
      ...
      const stagedGame = getRenderedGame();
      expect(stagedGame?.player.coord).toEqual({ q: 1, r: 0 });
      expect(stagedGame?.player.coord).not.toEqual({ q: 2, r: 0 });
      expect(stagedGame?.combat?.started).toBe(true);
      expect(stagedGame?.combat?.engagement).toMatchObject({
        autoStepOnVictory: true,
