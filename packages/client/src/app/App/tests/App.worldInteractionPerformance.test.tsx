...
describe('App world interaction performance', () => {
  describe('hover analysis worker controller', () => {
    ...
    it('skips tile lookup and path work for unrevealed distant hover targets', async () => {
      await withMockedWorldHoverSource({
        async configureSource({
          createWorldHoverInteractions,
          createEmptyWorldHoverSnapshot,
          ...
          const game = createGame(3, 'hover-hidden-target-seed');
          ...
          try {
            getResolvedTileAtSpy.mockClear();
            source.analyze.mockClear();

            controller.queuePointerMove({ clientX: 480, clientY: 240 });
            await flushAnimationFrame();

            expect(getResolvedTileAtSpy).not.toHaveBeenCalled();
            expect(source.analyze).not.toHaveBeenCalled();
            expect(hoverSnapshotRef.current.target).toEqual({
              q: WORLD_REVEAL_RADIUS + 2,
              r: 0,
            });
            expect(hoverSnapshotRef.current.clickable).toBe(false);
          } finally {
            controller.dispose();
            hexAtPointSpy.mockRestore();
